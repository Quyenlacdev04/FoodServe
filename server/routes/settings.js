...
const fallbackImages = {
  restaurant: 'https://images.unsplash.com/photo-1552566626-52f8b828add9?auto=format&fit=crop&w=800&q=80',
  food: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=800&q=80'
};

const getFoodImageFallback = (name) => {
  const lowercase = (name || '').toLowerCase();
  if (lowercase.includes('lẩu') || lowercase.includes('nấm')) {
  ...
  if (lowercase.includes('cơm')) {
    return 'https://images.unsplash.com/photo-1626804475297-4160aeeba1fa?w=400';
  }
  if (lowercase.includes('phở') || lowercase.includes('bún')) {
    return 'https://images.unsplash.com/photo-1582878826629-29b7ad1cb438?w=400';
  }
  if (lowercase.includes('sushi') || lowercase.includes('sashimi')) {
    return 'https://images.unsplash.com/photo-1579871494447-9811cf80d66c?w=400';
  }
  if (lowercase.includes('cà phê') || lowercase.includes('trà') || lowercase.includes('nước') || lowercase.includes('pepsi') || lowercase.includes('soju') || lowercase.includes('mót')) {
    return 'https://images.unsplash.com/photo-1556679343-c7306c1976bc?w=400';
  }
...
const getRestaurantImageFallback = (name) => {
  const lowercase = (name || '').toLowerCase();
  if (lowercase.includes('phở') || lowercase.includes('thìn')) {
    return {
  ...
  if (lowercase.includes('cơm') || lowercase.includes('tấm') || lowercase.includes('ghiền')) {
    return {
  ...
  if (lowercase.includes('highlands') || lowercase.includes('coffee') || lowercase.includes('cà phê')) {
    return {
  ...
  if (lowercase.includes('kfc') || lowercase.includes('gà rán')) {
    return {
  ...
  if (lowercase.includes('sushi') || lowercase.includes('hokkaido')) {
    return {
  ...
  if (lowercase.includes('don') || lowercase.includes('chicken')) {
    return {
  ...
  if (lowercase.includes('chay') || lowercase.includes('ngộ nhận')) {
    return {
  ...
  return {
    image: fallbackImages.restaurant,
    cover: fallbackImages.restaurant
  };
};
...
// GET /api/settings/fix-images - Chuyển toàn bộ ảnh uploads bị mất thành ảnh Unsplash chất lượng cao
router.get('/fix-images', async (req, res) => {
  try {
    let fixedRestaurants = 0;
    let fixedMenuItems = 0;

    const restaurants = await Restaurant.find();
    for (const r of restaurants) {
      let updated = false;
      const fallbacks = getRestaurantImageFallback(r.name);
      
      if (!r.image || r.image.includes('/uploads/') || r.image.includes('localhost') || r.image.includes('foodserve-z5vh')) {
        r.image = fallbacks.image;
        updated = true;
      }
      if (!r.cover || r.cover.includes('/uploads/') || r.cover.includes('localhost') || r.cover.includes('foodserve-z5vh')) {
        r.cover = fallbacks.cover;
        updated = true;
      }
    ...
    for (const item of menuItems) {
      if (!item.image || item.image.includes('/uploads/') || item.image.includes('localhost') || item.image.includes('foodserve-z5vh')) {
        item.image = getFoodImageFallback(item.name);
        await item.save();
        fixedMenuItems++;
      }
    }
    ...
    res.json({
      success: true,
      message: `Sửa ảnh thành công! Đã chuyển ${fixedRestaurants} nhà hàng và ${fixedMenuItems} món ăn sang ảnh Unsplash hoạt động.`,
      fixedRestaurants,
      fixedMenuItems
    });
  } catch (error) {
    console.error('Fix images error:', error);
    res.status(500).json({ message: 'Lỗi server khi sửa ảnh', error: error.message });
  }
});

// GET /api/settings/seed-db - Re-seed cơ sở dữ liệu với dữ liệu gốc sạch
router.get('/seed-db', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Đã re-seed DB thành công! Tạo mới ${countRestaurants} nhà hàng và ${countMenus} món ăn với ảnh Unsplash sạch.`,
      countRestaurants,
      countMenus
    });
  } catch (error) {
    console.error('Seed DB error:', error);
    res.status(500).json({ message: 'Lỗi server khi seed DB', error: error.message });
  }
});
