...
import { FiPlus, FiEdit2, FiTrash2, FiStar, FiClock, FiX, FiArrowLeft, FiImage, FiUpload } from 'react-icons/fi'
...
export default function AdminRestaurants() {
  ...
  const [formData, setFormData] = useState({
    name: '', image: '', categories: '', rating: 5.0,
  ...
  const [menuForm, setMenuForm] = useState({
    name: '', price: '', image: '', description: '', category: '', popular: false
  })
  ...
  const handleMenuImageUpload = async (e) => {
    ...
    if (url) setMenuForm(prev => ({ ...prev, image: url }))
  }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.ok) { toast.success('Đã xóa nhà hàng'); setRestaurants(restaurants.filter(r => r._id !== id)) }
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (res.ok) { toast.success(editingId ? 'Đã cập nhật nhà hàng!' : 'Đã thêm nhà hàng mới!'); setIsModalOpen(false); fetchRestaurants() }
      else toast.error('Lỗi khi lưu nhà hàng')
    } catch (err) { toast.error('Lỗi kết nối') }
  }
  ...
  const handleDeleteMenuItem = async (itemId) => {
    ...
    try {
      ...
      if (res.ok) { toast.success('Đã xóa món ăn'); setMenuItems(menuItems.filter(m => m._id !== itemId)) }
      else toast.error('Lỗi khi xóa món ăn')
    } catch (err) { toast.error('Lỗi kết nối') }
  }
  ...
  const handleSaveMenuItem = async (e) => {
    ...
    try {
      ...
      let res
      if (editingMenuItem) {
        res = await fetch(`http://localhost:5000/api/restaurants/menu/${editingMenuItem._id}`, {
          method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
        })
      } else {
        res = await fetch(`http://localhost:5000/api/restaurants/${selectedRestaurant._id}/menu`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
        })
      }
      if (res.ok) {
        toast.success(editingMenuItem ? 'Đã cập nhật món ăn!' : 'Đã thêm món ăn mới!')
        setIsMenuModalOpen(false)
        fetchMenuItems(selectedRestaurant._id)
      } else toast.error('Lỗi khi lưu món ăn')
    } catch (err) { toast.error('Lỗi kết nối') }
  }

  const openAddModal = () => {
    setEditingId(null)
    setFormData({ name: '', image: '', categories: '', rating: 5.0, deliveryTime: '15-30', distance: 2.5, minOrder: 0, discount: '' })
    setIsModalOpen(true)
  }
      ...
      <div className="bg-white dark:bg-dark-100 rounded-2xl shadow-card overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => setSelectedRestaurant(null)} className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-dark-200 transition-colors">
          ...
          {menuLoading ? (
            ...
            >
              ...
              <div className="relative aspect-[4/3] bg-gray-100 dark:bg-dark-300">
                ...
                />
                {/* Overlay nút thay ảnh */}
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button
                    onClick={() => openEditMenuModal(item)}
                    className="px-3 py-1.5 bg-white text-gray-800 text-xs font-bold rounded-lg hover:bg-gray-100 flex items-center gap-1"
                  >
        ...
        <AnimatePresence>
          {isMenuModalOpen && (
            <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
              ...
              >
                <button onClick={() => setIsMenuModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
                  <FiX size={24} />
                </button>
                <h2 className="text-xl font-bold dark:text-white mb-5">
                  {editingMenuItem ? '✏️ Sửa món ăn' : '➕ Thêm món ăn mới'}
                </h2>

                <form onSubmit={handleSaveMenuItem} className="space-y-4">
                  {/* Upload ảnh */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Hình ảnh món ăn</label>
                    <div className="relative">
                      {menuForm.image ? (
                        <div className="relative rounded-xl overflow-hidden aspect-video mb-2">
                          ...
                          <button type="button" onClick={() => setMenuForm(p => ({ ...p, image: '' }))}
                            className="absolute top-2 right-2 w-7 h-7 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600">
                            <FiX size={14} />
                          </button>
                        </div>
                      ) : (
                        <div onClick={() => menuFileInputRef.current?.click()}
                          className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center cursor-pointer hover:border-primary-500 hover:bg-primary-50/5 transition-colors">
                          {uploadingImage ? (
                            ...
                            <>
                              ...
                              <p className="text-xs text-gray-300 mt-1">PNG, JPG tối đa 5MB</p>
                            </>
                          )}
                        </div>
                      )}
                    ...
                    {/* Hoặc nhập URL */}
                    <div className="mt-2">
                      <input type="url" placeholder="Hoặc nhập URL ảnh..." value={menuForm.image}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input type="text" value={menuForm.category} onChange={e => setMenuForm(p => ({ ...p, category: e.target.value }))}
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    <button type="button" onClick={() => setIsMenuModalOpen(false)} className="px-5 py-2 rounded-xl font-semibold text-gray-500 hover:bg-gray-100 dark:hover:bg-dark-100 transition-colors">Hủy</button>
                    <button type="submit" disabled={uploadingImage} className="btn-primary py-2 px-6 disabled:opacity-50">
                      {uploadingImage ? 'Đang upload...' : 'Lưu món ăn'}
                    </button>
                  </div>
                </form>
              </motion.div>
    ...
    <div className="bg-white dark:bg-dark-100 rounded-2xl shadow-card overflow-hidden">
      <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400">Click vào nhà hàng để quản lý món ăn & hình ảnh</p>
        </div>
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div key={restaurant._id} className="border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden hover:shadow-lg transition-shadow group relative bg-white dark:bg-dark-200 cursor-pointer"
            onClick={() => handleViewMenu(restaurant)}
          >
            <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2 z-10">
              <button onClick={e => { e.stopPropagation(); openEditModal(restaurant) }}
                className="w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center hover:bg-blue-600 shadow-lg" title="Sửa nhà hàng">
                <FiEdit2 size={14} />
              </button>
              <button onClick={e => { e.stopPropagation(); handleDelete(restaurant._id) }}
                className="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 shadow-lg" title="Xóa">
                <FiTrash2 size={14} />
              </button>
            </div>
            ...
            <div className="aspect-[4/3] overflow-hidden relative">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <span className="text-white font-bold text-sm bg-white/20 px-4 py-2 rounded-xl backdrop-blur-sm">
                  🍽️ Quản lý món ăn
                </span>
              </div>
            </div>
            ...
            <div className="p-4">
              <h4 className="font-bold text-base dark:text-white mb-1 line-clamp-1">{restaurant.name}</h4>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div>
                  ...
                  {formData.image ? (
                    ...
                    <div onClick={() => fileInputRef.current?.click()}
                      ...
                      {uploadingImage ? <p className="text-sm text-gray-400">Đang upload...</p> : (
                        <><FiUpload className="mx-auto text-2xl text-gray-300 mb-1" /><p className="text-sm text-gray-400">Click để tải ảnh lên</p></>
                      )}
                    </div>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="number" step="0.1" min="1" max="5" required value={formData.rating} onChange={e => setFormData({...formData, rating: parseFloat(e.target.value)})}
                      className="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 dark:text-white focus:ring-2 focus:ring-primary-500 outline-none" />
                  </div>
                  <div>
